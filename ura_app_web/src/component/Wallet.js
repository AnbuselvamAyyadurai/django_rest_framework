import React, { Component } from 'react';
import { SearchBar, Header, List } from 'react-native-elements';
import Send from './Send';
import { Clipboard } from "react-native";
import Drawer from 'react-native-drawer';
import SideBarContent from './SideBarContent';
import Share, { ShareSheet, Button } from 'react-native-share';
import axios from 'axios';
import { Fonts } from './Fonts';
import QRCode from 'react-native-qrcode';
// import PTRView from 'react-native-pull-to-refresh';


import {
    Platform,
    StyleSheet,
    Text,
    View,
    AsyncStorage,
    TouchableHighlight,
    TouchableOpacity,
    AppRegistry,
    Image,
    TextInput,
    FlatList,
    Dimensions,
    ScrollView,


} from 'react-native';
import Modal from 'react-native-modalbox';
import { wallet } from 'ethers';
import LinearGradient from 'react-native-linear-gradient';
import { connect } from 'react-redux';
import * as action from './action/action';
import Constant from '../Constants/Constant';
var screen = Dimensions.get('window');
class Wallet extends Component {

    static navigationOptions = {

        headerLeft: <TouchableOpacity onPress={this.openControlPanel}>
            <Image source={require('../Image/sidemenu.png')}
                style={{ width: 40, height: 35, resizeMode: 'contain' }} />
        </TouchableOpacity>,
        headerTitle: <SearchBar
            placeholder="Search"
            icon={{ type: 'evilicon', name: 'search', color: 'green' }}
            clearIcon={{ name: 'close-o', color: 'red', }}
            iconStyle={{
                width: 50,
                height: 50,
                resizeMode: 'contain'
            }}
            inputStyle={{
                backgroundColor: '#dee0e2',
                fontSize: 18,
                color: '#000000',
                borderRadius: 10
            }}
            containerStyle={{
                backgroundColor: '#ffffff',
                width: '100%'

            }}


        />,
        headerRight: <TouchableOpacity >
            <Image source={require('../Image/message.png')}
                style={{ width: 40, height: 35, resizeMode: 'contain' }} />
        </TouchableOpacity>,


        // headerTitleStyle: { textAlign: 'center', alignSelf: 'center' },
        headerStyle: {
            backgroundColor: '#ffffff',
            // shadowColor: 'transparent',
            // backgroundColor: '#fff',
            elevation: 0,

        }

    };


    constructor(props) {
        super(props);
        this.state = {
            visible: false,// share screen visible
            loginData: {},
            text: '',
            status: true,
            accNo: '',
            names: []
        }
    }

    onCancel() {
        console.log("CANCEL")
        this.setState({ visible: false });
    }
    onOpen() {
        console.log("OPEN")
        this.setState({ visible: true });
    }
    Qrcode() {
        var acc = this.state.loginData.accounts.accNumber;
        console.log("val", acc);
        this.setState({ accNo: acc });
        // this.setState({ accNo: acc });
        console.log("accounts", accNo);
    }
    Qrcopy() {
        Clipboard.setString(this.state.accNo);
        this.refs.URAaddress.close(this.refs.copiedmodel.open())

    }
    componentDidMount() {
        var that = this;
        that.balance();
        let ethers = require('ethers');
        let utils = require('ethers/utils');
        var logDetails = that.props.LoginData.data;
        console.log('logDetailsacc', logDetails);

        console.log('ID', logDetails.accounts.userId);
        var userId = logDetails.accounts.userId;
        console.log('userId', userId);
        //   var privateKey = '0x57933432446b0ecd2d0c0a4efa5291d093168c748969d31f629337e870e18102';
        axios({
            method: "GET",
            url: Constant.base_url + '/transactions/detalis/' + userId,
            Headers: {
                "Content-Type": "application/json",
            }
        }).then(({ data }) => {
            console.log("detailseee", data);
            that.setState({ names: data.transactions })
            console.log('names', that.state.names)

        }).catch((err) => {
            console.log("error", err);
        });

        //   var privateKey = '0x57933432446b0ecd2d0c0a4efa5291d093168c748969d31f629337e870e18102';
        AsyncStorage.getItem('loginUser').then((value) => {
            var userLogin = JSON.parse(value);
            console.log("Async login user: " + userLogin)
            this.setState({ loginData: userLogin })

            var privateKey = this.state.loginData.accounts.privateKey;
            var wallet = new ethers.Wallet(privateKey);

            wallet.provider = ethers.providers.getDefaultProvider('ropsten');
            // var that = this;
            wallet.getBalance().then(function (balance) {

                // balance is a BigNumber (in wei); format is as a sting (in ether)
                var etherString = utils.formatEther(balance);
                //this.refs.confirm.close(this.refs.okaybtn.open());
                console.log("Balance: " + etherString);
                var number = parseFloat(etherString);
                console.log("val", number);
                var eth = number * 454.22;
                number = number.toFixed(2);
                that.setState({ number });
                that.setState({ ethbalance: eth.toFixed(2) });
                console.log("statebal: " + eth);

                console.log("Balance: " + number);
                that.Qrcode();



            });

        })



    }
    balance() {
        var that = this;
        let ethers = require('ethers');
        let utils = require('ethers/utils');
        // var logDetails = that.props.LoginData.data;
        // console.log('logDetailsacc', logDetails);

        // console.log('ID', logDetails.accounts.userId);
        // var userId = logDetails.accounts.userId;
        // console.log('userId', userId);
        // //   var privateKey = '0x57933432446b0ecd2d0c0a4efa5291d093168c748969d31f629337e870e18102';
        // axios({
        //     method: "GET",
        //     url: Constant.base_url + '/transactions/detalis/' + userId,
        //     Headers: {
        //         "Content-Type": "application/json",
        //     }
        // }).then(({ data }) => {
        //     console.log("detailseee", data);
        //     that.setState({ names: data.transactions })
        //     console.log('names', that.state.names)
        // }).catch((err) => {
        //     console.log("error", err);
        // });

        AsyncStorage.getItem('loginUser').then((value) => {
            var userLogin = JSON.parse(value);
            console.log("Async login user: " + userLogin)
            this.setState({ loginData: userLogin })

            var privateKey = this.state.loginData.accounts.privateKey;
            var wallet = new ethers.Wallet(privateKey);

            wallet.provider = ethers.providers.getDefaultProvider('ropsten');

            wallet.getBalance().then(function (balance) {

                // balance is a BigNumber (in wei); format is as a sting (in ether)
                var etherString = utils.formatEther(balance);
                //this.refs.confirm.close(this.refs.okaybtn.open());
                console.log("Balance: " + etherString);
                var number = parseFloat(etherString);
                console.log("val", number);
                var eth = number * 454.22;
                number = number.toFixed(2);
                that.setState({ number });
                that.setState({ ethbalance: eth.toFixed(2) });
                console.log("statebal: " + eth);
                console.log("Balance: " + number);

            });

        })

    }


    render() {
        let shareOptions = {
            title: "Top to share with AirDrop",
            message: this.state.accNo,
            url: "http://facebook.github.io/react-native/",
            subject: "URAllowance" //  for email
        };
        let shareImageBase64 = {
            title: "Top to share with AirDrop",
            message: this.state.accNo,
            url: '',
            subject: "URAllowance" //  for email
        };
        // const { navigate } = this.props.navigation;

        return (

            <View style={styles.container} >
                {/* <PTRView onRefresh={this.balance.bind(this)} > */}
                <View style={{ height: 300, backgroundColor: '#ffffff', padding: 10 }}>
                    <LinearGradient colors={['#6887ff', '#2952ea',]} style={styles.LinearGradientStyle}>
                        <View style={styles.balanceview}>
                            <View style={styles.uravalue}>
                                <Text style={styles.uravaluetext}>{this.state.number}</Text>
                                <Text style={styles.uravaluetext}> URA</Text>
                            </View>
                            <View style={styles.usdvalue}>
                                <Text style={styles.usdvaluetext} >${this.state.ethbalance}</Text>
                                <Text style={styles.usdvaluetext} > USD</Text>
                            </View>
                        </View>
                    </LinearGradient>
                    <View style={{ flexDirection: 'row', marginTop: 15, justifyContent: 'center' }}>

                        <View style={styles.wallettransactionoptions}>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                <TouchableOpacity onPress={() => this.props.navigation.navigate('Send')}>
                                    <Image source={require('../Image/send.png')}
                                        style={{ width: 30, height: 30, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }} >
                                <Text style={styles.wallettext} onPress={() => this.props.navigation.navigate('Send')}> Send </Text>
                            </View>
                        </View>
                        <View style={styles.wallettransactionoptions}>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                <TouchableOpacity onPress={() => this.props.navigation.navigate('Request')}>
                                    <Image source={require('../Image/Request.png')}
                                        style={{ width: 30, height: 30, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }} >
                                <Text style={styles.wallettext} onPress={() => this.props.navigation.navigate('Request')} > Request </Text>
                            </View>
                        </View>
                        <View style={styles.wallettransactionoptions}>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                <Image source={require('../Image/Transactions.png')}
                                    style={{ width: 30, height: 30, resizeMode: 'contain' }} />
                            </View>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }} >
                                <Text style={styles.wallettext}> Transactions </Text>
                            </View>
                        </View>
                        <View style={styles.wallettransactionoptions}>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }} >
                                <TouchableOpacity onPress={() => this.refs.Moreoptions.open()}>
                                    <Image source={require('../Image/more.png')}
                                        style={{ width: 30, height: 30, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                <Text style={styles.wallettext} onPress={() => this.refs.Moreoptions.open()}>More </Text>
                            </View>
                        </View>
                    </View>
                </View>


                <View style={{ height: 300, marginTop: '3%', marginRight: '3%', marginLeft: '3%', backgroundColor: '#ffffff' }}>
                    <Text style={{ fontSize: 26, color: '#000000', fontWeight: '600', paddingBottom: 5 }} > Recent Activity </Text>
                    <View style={{ height: 300, }}>
                        <FlatList showsVerticalScrollIndicator={false}
                            data={this.state.names}
                            renderItem={({ item, index }) =>
                                <TouchableOpacity>
                                    <View style={{ flexDirection: 'row' }}>
                                        <View style={{ width: '20%', justifyContent: 'center', alignItems: 'center' }}>
                                            <TouchableOpacity >
                                                <Image source={{ uri: item.picture }} style={styles.roleImg} />
                                            </TouchableOpacity>
                                        </View>
                                        <View style={{ width: '80%', borderBottomWidth: 0.3, flexDirection: 'row' }}>
                                            <View style={{ width: '60%', justifyContent: 'center' }}>
                                                <Text style={styles.text}>{item.transType} URA Coins</Text>
                                                <Text style={styles.username}>{item.transType == 'Sent' ? "to" : "from"} {item.name}</Text>
                                                <Text style={styles.username1}>{item.date} at {item.time}</Text>
                                            </View>
                                            <View style={{ width: '40%', alignItems: 'flex-end' }}>
                                                <Text style={item.transType == 'Sent' ? styles.text1 : styles.text2}> {item.transType == 'Sent' ? "-" : "+"}{item.ura.toFixed(3)} <Text> URA  </Text></Text>
                                                <Text style={{ fontFamily: 'Avenir LT Std', fontWeight: '600' }}>{item.transType == 'Sent' ? "-" : ""}${item.usd} <Text> USD  </Text></Text>
                                                {/* <Text style={{ fontFamily: 'Avenir LT Std', fontWeight: 'bold' }}> pending</Text> */}
                                            </View>
                                        </View>
                                    </View>
                                </TouchableOpacity>
                            }
                        />


                    </View>
                </View>


                {/* More Options in Wallet */}

                <Modal backdropPressToClose={false} style={styles.moreoptionsmodelbox} position={'bottom'} ref={'Moreoptions'} swipeArea={20}>
                    <View style={{ flex: 1 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'center', borderBottomWidth: 1, padding: 10 }} >
                            <View style={{ width: '90%', justifyContent: 'center' }}>
                                <Text style={{ fontFamily: Fonts.AvenirLTStdRoman, color: "#000000", fontSize: 20, }}>More Options</Text>
                            </View>
                            <View style={{ width: '10%', justifyContent: 'center' }}>
                                <TouchableOpacity onPress={() => this.refs.Moreoptions.close()}>
                                    <Image source={require('../Image/closeicon.png')}
                                        style={{ width: 24, height: 24, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'center', borderBottomWidth: 1, padding: 15 }}  >
                            <View style={{ width: '10%', justifyContent: 'center' }}>
                                <TouchableOpacity >
                                    <Image source={require('../Image/QRcode.png')}
                                        style={{ width: 35, height: 35, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                            <View style={{ width: '90%', justifyContent: 'center' }}>
                                <Text style={{ fontFamily: Fonts.AvenirLTStdBlack, color: "#000000", fontSize: 25, }}
                                    onPress={() => this.refs.Moreoptions.close(this.refs.URAaddress.open())}> My URA Wallet Address </Text>
                            </View>

                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'center', borderBottomWidth: 1, padding: 15 }}  >
                            <View style={{ width: '10%', justifyContent: 'center' }}>
                                <TouchableOpacity >
                                    <Image source={require('../Image/Lock_wallet.png')}
                                        style={{ width: 35, height: 35, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                            <View style={{ width: '90%', justifyContent: 'center' }}>
                                <Text style={{ fontFamily: Fonts.AvenirLTStdBlack, color: "#000000", fontSize: 25, }}> Lock Wallet </Text>
                            </View>
                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'center', borderBottomWidth: 1, padding: 15 }}  >
                            <View style={{ width: '10%', justifyContent: 'center' }}>
                                <TouchableOpacity >
                                    <Image source={require('../Image/ChangePin.png')}
                                        style={{ width: 35, height: 35, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                            <View style={{ width: '90%', justifyContent: 'center' }}>
                                <Text style={{ fontFamily: Fonts.AvenirLTStdBlack, color: "#000000", fontSize: 25, }}> Change PIN</Text>
                            </View>
                        </View>
                    </View>
                </Modal>

                {/* My URA Wallet Address  */}

                <Modal backdropPressToClose={false} style={styles.URAaddressmodelbox} position={'center'} ref={'URAaddress'} swipeArea={20}>
                    <View style={{ flex: 1 }}>
                        <View>
                            <View style={{ width: '100%', justifyContent: 'flex-end', alignItems: 'flex-end', marginTop: '3%' }}>
                                <TouchableOpacity onPress={() => this.refs.URAaddress.close()}>
                                    <Image source={require('../Image/closeicon.png')}
                                        style={{ width: 20, height: 20, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: '2%' }}>
                            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                                <Text style={{ fontFamily: Fonts.AvenirLTStdRoman, color: '#000000', fontSize: 21, textAlign: 'center', paddingBottom: 25 }}>
                                    {"Copy or Share this address to \nanother URAllowance user."}</Text>
                            </View>
                        </View>
                        <View >
                            {/* QR code image */}
                            <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', paddingBottom: 25 }}>
                                {/* <TouchableOpacity>
                                    <Image source={require('../Image/QRcode.png')}
                                        style={{ width: 200, height: 200, resizeMode: 'contain' }} />
                                </TouchableOpacity> */}
                                <View style={{ borderWidth: 1, borderColor: '#000', padding: 5 }}>
                                    <QRCode
                                        value={this.state.accNo}
                                        size={150}
                                        bgColor='black'
                                        fgColor='#ffffff' />
                                </View>
                            </View>
                        </View>

                        <View style={{ alignItems: 'center', justifyContent: 'center' }} >
                            <TouchableOpacity
                                style={styles.copywalletbtn} onPress={() => this.Qrcopy()}>
                                <Text style={{ color: 'blue', textAlign: 'center', fontSize: 20, padding: 15, fontFamily: Fonts.AvenirLTStdBook, }}
                                > Copy Wallet Address </Text>
                            </TouchableOpacity>
                        </View>
                        <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: '7%' }} >
                            <TouchableOpacity onPress={() => {
                                Share.open(shareImageBase64)
                            }}
                                style={styles.sharewalletbtn}  >
                                <Text style={{ color: 'blue', textAlign: 'center', fontSize: 20, padding: 15, fontFamily: Fonts.AvenirLTStdRoman }}
                                > Share Wallet Address </Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </Modal>

                <Modal style={styles.copiedmodelbox} position={'center'} ref={'copiedmodel'} swipeArea={20}>
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                        <View style={{ alignItems: 'center', justifyContent: 'center', }}>
                            <Text style={{ color: 'lightgreen', fontFamily: Fonts.AvenirLTStdBlack, fontSize: 26, textAlign: 'center' }}> Copied! </Text>
                        </View>

                        <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: '3%' }}>
                            <Text style={{ fontFamily: Fonts.AvenirLTStdRoman, color: "#000000", fontSize: 21, textAlign: 'center' }}>
                                {"Your URA Wallet Address has been \ncopied to your clipboard"}</Text>

                        </View>
                    </View>
                </Modal>

                {/* Shared screen icon and name */}

                <ShareSheet visible={this.state.visible} onCancel={this.onCancel.bind(this)}>
                    <Button iconSrc={{ uri: TWITTER_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "twitter"
                                }));
                            }, 300);
                        }}>Twitter</Button>
                    <Button iconSrc={{ uri: FACEBOOK_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "facebook"
                                }));
                            }, 300);
                        }}>Facebook</Button>
                    <Button iconSrc={{ uri: WHATSAPP_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "whatsapp"
                                }));
                            }, 300);
                        }}>Whatsapp</Button>
                    <Button iconSrc={{ uri: GOOGLE_PLUS_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "googleplus"
                                }));
                            }, 300);
                        }}>Google +</Button>
                    <Button iconSrc={{ uri: EMAIL_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "email"
                                }));
                            }, 300);
                        }}>Email</Button>
                    <Button
                        iconSrc={{ uri: CLIPBOARD_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                if (typeof shareOptions["url"] !== undefined) {
                                    Clipboard.setString(shareOptions["url"]);
                                    if (Platform.OS === "android") {
                                        ToastAndroid.show('Link copiado al portapapeles', ToastAndroid.SHORT);
                                    } else if (Platform.OS === "ios") {
                                        AlertIOS.alert('Link copiado al portapapeles');
                                    }
                                }
                            }, 300);
                        }}>Copy Link</Button>
                    <Button iconSrc={{ uri: MORE_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.open(shareOptions)
                            }, 300);
                        }}>More</Button>
                </ShareSheet>
                {/* </PTRView > */}
            </View>

        );
    }
}



const styles = StyleSheet.create({
    container: {
        flex: 1,
    },
    balanceview: {
        flexDirection: 'column',
        width: "100%",
        height: '70%',
        // backgroundColor: '#3742fa',
        borderRadius: 10,
        justifyContent: 'center',
        alignItems: 'center'
    },
    uravalue: {
        marginTop: 43,
        justifyContent: 'center',
        alignItems: 'center',
        flexDirection: 'row'
    },
    uravaluetext: {
        fontFamily: 'Avenir LT Std',
        fontWeight: '900',
        fontSize: 50,
        color: '#ffffff'
    },
    usdvalue: {
        justifyContent: 'center',
        alignItems: 'center',
        flexDirection: 'row'
    },
    usdvaluetext: {
        fontSize: 18,
        color: '#fff',
        fontFamily: 'Avenir LT Std',
    },
    wallettext: {
        fontSize: 14,
        color: '#000000',
        fontFamily: Fonts.AvenirLTStdBlack,

    },
    wallettransactionoptions: {
        width: "25%",
        height: 50,
        // margin: 5, 
        alignItems: 'center',
        flexDirection: 'column',
        justifyContent: 'center'
    },
    userimage: {
        // marginTop: 30,
        width: 50,
        height: 50,
        borderRadius: 50 / 2,
    },
    roleImg: {
        height: 50,
        width: 50,
        borderRadius: 50 / 2,
        // resizeMode: 'contain',
    },
    text: {
        color: '#000000',
        fontFamily: 'Avenir LT Std',
        fontSize: 19,
        fontWeight: '600'
    },
    username: {
        fontFamily: Fonts.AvenirLTStdBook,
        fontSize: 21,
    },
    username1: {
        fontFamily: Fonts.AvenirLTStdBook,
        fontSize: 12.5,
    },
    settimes: {
        fontFamily: 'Avenir LT Std',
        fontSize: 12,
    },
    text1: {
        fontWeight: '600',
        color: '#F44336',
        fontFamily: 'Avenir LT Std',
        fontSize: 18,
    },
    text2: {
        fontWeight: '600',
        color: '#09bc03',
        fontFamily: 'Avenir LT Std',
        fontSize: 18,
    },
    profileimage: {
        resizeMode: 'contain',
        width: 50,
        height: 50,
        borderRadius: 50 / 2,
    },
    moreoptionsmodelbox: {
        justifyContent: 'center',
        alignItems: 'center',
        height: '46%',
        overflow: 'hidden',
        position: 'absolute',
        borderTopLeftRadius: 20,
        borderTopRightRadius: 20
    },
    URAaddressmodelbox: {
        justifyContent: 'center',
        alignItems: 'center',
        height: '90%',
        width: '80%',
        borderRadius: 20
    },

    copiedmodelbox: {
        justifyContent: 'center',
        alignItems: 'center',
        height: '45%',
        width: '80%',
        borderRadius: 20
    },
    copywalletbtn: {
        borderWidth: 1,
        borderColor: 'blue',
        // borderTopColor: 'blue',
        // borderTopWidth: 0.5,
        borderRadius: 20,
        // borderBottomColor: 'blue',
        // borderBottomWidth: 0.7,
        // borderBottomEndRadius: 20,
        // borderBottomLeftRadius: 20,
        // borderBottomRightRadius: 20,
        // borderBottomStartRadius: 20,
        // borderTopEndRadius: 20,
        // borderTopLeftRadius: 20,
        // borderTopRightRadius: 20,
        // borderTopStartRadius: 20,
        // borderLeftColor: 'blue',
        // borderLeftWidth: 0.1,
        // borderRadius
        // borderRightWidth: 0.1
    },
    sharewalletbtn: {
        borderWidth: 1,
        borderColor: 'blue',
        borderRadius: 20,

    }
});

//  twitter icon
const TWITTER_ICON = "data:image/png;base64,";

//  facebook icon
const FACEBOOK_ICON = "data:image/png;base64,";

//  whatsapp icon
const WHATSAPP_ICON = "data:image/png;base64,";

//  gplus icon
const GOOGLE_PLUS_ICON = "data:image/png;base64,";

//  email icon
const EMAIL_ICON = "data:image/png;base64,";

//  clipboard icon
const CLIPBOARD_ICON = "data:image/png;base64,";

//  more icon
const MORE_ICON = "data:image/png;base64,";


const REACT_ICON = 'data:image/png;base64,';

AppRegistry.registerComponent('Wallet', () => Wallet);
const StateToProps = (state) => {
    return {
        LoginData: state.LoginUser,
        data: state.data,

    }
}

const DispatchToProps = (dispatch) => {
    return {

    }
}
export default connect(StateToProps, DispatchToProps)(Wallet);

